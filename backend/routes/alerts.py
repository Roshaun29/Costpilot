...
async def get_alerts(
    ...
    match_q = {"user_id": uid}
    if read is not None: match_q["read"] = read
    if channel is not None: match_q["channel"] = channel
...
async def mark_read(id: str, current_user: dict = Depends(get_current_user), db = Depends(get_db)):
    uid = current_user["_id"]
    res = await db.alerts.update_one({"_id": ObjectId(id), "user_id": uid}, {"$set": {"read": True}})
...
async def delete_alert(id: str, current_user: dict = Depends(get_current_user), db = Depends(get_db)):
    uid = current_user["_id"]
    res = await db.alerts.delete_one({"_id": ObjectId(id), "user_id": uid})
